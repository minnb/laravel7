          ...
          <div class="panel panel-default courseSidebar">
            <div class="panel-heading bg-color-1 border-color-1">
              ...
              <ul class="panel-body list-group">
                ...
                <li class="list-group-item"><a href="{{ route('get.home.tour.location.list',['cate'=>$cate_name->alias,'id'=>$item->id, 'name'=>Illuminate\Support\Str::slug($item->name).'.html'])}}">{!! $item->name !!}</a></li>
                @endforeach
              </ul>
          </div>
          @endif
	...
	<div class="col-md-9 col-md-pull-3 col-sm-7 col-sm-pull-5 col-xs-12">
        ...
        <div class="media courseList couresListPage">
          ...
          <div class="media-body">
            <h3 class="media-heading"><a href="{{ route('get.home.tour.detail',['cate'=>$cate_name->alias,'id'=>$item->id, 'name'=>Illuminate\Support\Str::slug($item->name).'.html'])}}" class="color-10">{!! $item->name !!}</a></h3>
            <ul class="list-inline">
              <li><i class="fa fa-clock-o" aria-hidden="true"></i><span class="v-margin-right-15">{!! getTourTime()[$item->base_unit] !!}</span></li>
              <?php $startDate = App\Models\ScheduleTour::getScheduleByTourId($item->id); ?>
              <li><i class="fa fa-calendar-o" aria-hidden="true"></i> Khở hành: {{ $startDate == "1990-01-01" ? "Liên hệ" : date("d/m/Y", strtotime($startDate)) }}</li>
            </ul>
