...
<form class="form-horizontal" role="form" action="{{ route('post.dashboard.pageSingle.create')}}" method="post" enctype="multipart/form-data">
    @csrf
    <div class="form-group">
        <label class="col-sm-2 control-label no-padding-right" for="form-field-1"> Title </label>
        <div class="col-sm-9">
            <input type="text" id="form-field-1" placeholder="Title" name="title" class="col-xs-10 col-sm-5" required="" value="{{ old('title')}}" />
        </div>
    </div>
    <div class="form-group">
        ...
        <div class="col-xs-10">
            <select multiple="" id="category" name="category[]" class="select2" data-placeholder="Click to Choose...">
                {!! getSelectArrayForm(App\Models\Categories::getSelect2Category(2), old('category', isset($data) ? convertStrToArr("|", $data['cate_id']): [0]) ) !!}
            </select>
        </div>
    </div>
    <div class="form-group">
