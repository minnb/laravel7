...
<form class="form-horizontal" role="form" action="{{ route('post.dashboard.social.edit', ['id'=>$data['id']])}}" method="post" enctype="multipart/form-data">
    @csrf
    <div class="form-group">
        ...
        <div class="col-xs-4">
            <select class="form-control" id="form-field-select-1" name="parent" required="">
                 <?php selectedOption(getSocial(), old('display', isset($data) ? $data['type'] : '')) ?>
            </select>
        </div>
    </div>
    <div class="form-group">
        ...
        <div class="col-xs-9">
            <textarea name="link" id="link" rows="6" class="col-xs-9 col-sm-5">{{  old('link', isset($data) ? $data['link'] : "")}}</textarea>
        </div>
    </div>
    <div class="form-group">
